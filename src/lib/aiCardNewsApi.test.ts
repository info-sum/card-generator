...
test('generate-cardnews handler uses the request GPT key before the environment key', async () => {
  ...
  try {
    ...
    assert.match(requests[0]?.body ?? '', /card-claude guidance for Korean card-news copy/)
    assert.match(requests[0]?.body ?? '', /first slide must be a hook-style cover/)
    assert.match(requests[0]?.body ?? '', /메인 문구/)
    assert.match(requests[0]?.body ?? '', /2~3줄/)
    assert.match(requests[0]?.body ?? '', /배경, 변화, 의미/)
    assert.match(requests[0]?.body ?? '', /대상, 수치, 사례, 조건/)
    assert.match(requests[0]?.body ?? '', /각 줄은 배경 → 변화\/핵심 → 활용\/영향/)
    assert.match(requests[0]?.body ?? '', /description and content2 should summarize each card more concretely/)
    assert.match(requests[0]?.body ?? '', /한 장에 하나의 핵심 메시지/)
